...
def _append_xlsx(records, xlsx_path):
    ...
    convention used by `pipeline.xlsx_merge._dedup_key` and
    `pipeline.merge_master._dedup_key`. Without this, a row written here
    with mixed-case URL collides with an existing lowercased remote URL
    only at merge time — that produces an intra-union shrink (e.g.
    103 → 102) which trips the xlsx_merge no-shrink CANARY and aborts
    safe_push entirely. Root cause of the 2026-06-15 NA-collector CI
    failure on the "Pending (pre-filter) merge would shrink (102 < ...)"
    assertion.
    """
    import openpyxl
    try:
    ...
    # IMPORTANT: lowercase + strip to match xlsx_merge._dedup_key.
    existing_urls = set()
    for sheet_name in ("Pending", "Recalls"):
        ...
        for row in sh.iter_rows(min_row=2):
            v = row[url_idx].value
            if v:
                existing_urls.add(str(v).strip().lower())

    appended = skipped = 0
    for rec in records:
        url_key = (rec.url or "").strip().lower()
        if url_key and url_key in existing_urls:
            skipped += 1
            continue
        ws.append(_row_for(rec, header))
        # Add lowercased key to prevent intra-batch case-variant duplicates
        # (the Google-News supplement can return the same article twice
        # under different query branches with slightly different casing in
        # query parameters or redirect chains).
        if url_key:
