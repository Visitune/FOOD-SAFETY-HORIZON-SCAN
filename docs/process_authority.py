...
# ──────────────────────────────────────────────────────────────────────
_GLOBAL_FRAMING = (
    "Across global jurisdictions the regulatory architecture differs but the "
    "engineering principle converges. In the United States, FDA 21 CFR 113 "
    "(thermally processed low-acid canned foods) and 21 CFR 114 (acidified "
    "foods) require a scheduled process established by a qualified process "
    "authority. In the European Union, Regulation 852/2004 imposes general "
    "hygiene and HACCP obligations on food-business operators, and "
    "Regulation 2073/2005 fixes microbiological criteria — neither is a "
    "direct equivalent of US LACF/acidified-food filing, but both presuppose "
    "process control validated against a documented hazard analysis, with "
    "national LACF/acidified-food rules layered on top in some Member "
...
def _fallback_botulinum(trigger: Dict[str, Any]) -> str:
    return (
        ...
        f"botulinum or botulism-relevant recalls. The control review for "
        f"such events is product-specific. For shelf-stable low-acid, "
        f"acidified, aseptic, or commercially sterile products, scheduled-"
        f"process adequacy under a qualified process authority is the "
        f"central control. For refrigerated anaerobic products — peeled "
        f"garlic, sous-vide, vacuum or modified-atmosphere packs, smoked "
        f"or fermented fish — the operative controls are validated "
        f"refrigeration discipline, water-activity, pH, salt content, "
        f"oxygen barrier integrity, and shelf-life qualification, with or "
        f"without a thermal step. {_GLOBAL_FRAMING} Recall events in this "
        f"category most commonly trace to one of: an unfiled or outdated "
        f"scheduled process; a deviation resolved without qualified "
        f"review; a formulation or packaging change introduced without "
        f"process re-evaluation; a temperature-abuse event in the cold "
        f"chain; or seal-integrity loss in anaerobic packaging. "
        f"Manufacturers producing low-acid canned, acidified, fermented, "
        f"or anaerobically packaged products — including refrigerated "
        f"anaerobic products — should confirm with a qualified process "
        f"authority that filings, multi-hurdle controls, GMP programmes, "
        f"and deviation handling remain current before any production "
        f"change."
    )
