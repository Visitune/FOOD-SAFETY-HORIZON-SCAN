...
def _is_clean_row(row: Dict[str, Any]) -> bool:
    ...
    #
    # Every time this fires, it means a row with a potentially fabricated
    # critical field was intercepted before it could be promoted.
    reason = str(row.get("Reason") or "").strip()
    if _pathogen_reason_class_mismatch(pathogen, reason):
        log.warning("hazard-class mismatch: Pathogen=%r Reason=%r URL=%r",
                     pathogen, reason[:120], row.get("URL", ""))
        return False
    return True
...
def _is_dead_url_reason(reason: str) -> bool:
    ...
    transient = any(t in r for t in (
        "ssl", "tls", "certificate", "network", "timeout", "connection",
        "http 5", "http 403", "http_error", "curl_cffi", "max retries",
    ))
    if transient:
        return False
    return any(t in r for t in ("http 404", "http 410"))
