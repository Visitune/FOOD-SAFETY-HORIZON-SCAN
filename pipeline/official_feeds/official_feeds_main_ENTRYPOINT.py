...
def run_source(code: str, max_age_days: int = 14, dry_run: bool = False,
    ...
    # Stage 3b flags records it could not resolve to a regulator URL
    # (_pending_no_auth_url=True) but does nothing to stop them; without
    # this gate those rows (e.g. facebook.com news mirrors) get written to
    # xlsx in Stage 4. Run EVERY accepted record through the shared
    # check_gap_finder_row allowlist guard and drop anything that is not a
    # regulator/authority URL. stdlib-only import; wrapped so a guard
    # failure can never crash the collector.
    try:
