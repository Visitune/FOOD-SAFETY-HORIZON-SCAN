...
_PATHOGEN_SYNONYMS = {
    ...
    # Audit 2026-05-15: source feeds emit at least nine distinct STEC
    # surface forms (English, French, with and without parens, with and
    # without serogroup). All must collapse to one canonical so the
    # monthly distribution table and the M-narrative agree. The
    # regex-based catch-all in _consolidate_pathogen_label() backstops
    # any future variant.
    ...
    # to hygiene-indicator counts (non-pathogenic), and lumping them
    # into the STEC bucket would inflate STEC counts. Keep them as
    # their own bare labels; the consolidator's regex pass only
    # triggers on explicit STEC / shiga-toxin / VTEC tokens, so
    # bare-E.coli is safe.
...
def _consolidate_pathogen_label(label: str) -> str:
    ...
    Resolution order (audit 2026-05-15, mirrors build_weekly_report_afts.py):
    ...
    if re.search(r"\b(stec|shiga[\s\-]?toxin|shigatoxin|shigatoxinogène|vtec)\b",
                 s, re.I):
        return "E. coli STEC"
    # 4) Bare Salmonella collapse (preserves serovars)
    if re.fullmatch(r"salmonella(\s+spp\.?)?", s, re.I):
        return "Salmonella spp."
    # 5) Best-effort: paren-stripped base form
...
<div class="site-nav">
  ...
  <div class="links">
    <a href="index.html"><svg class="ic" viewBox="0 0 16 16" width="10" height="10" fill="currentColor"><circle cx="8" cy="8" r="6"/></svg>Live Dashboard</a>
    <a href="weekly.html"><svg class="ic" viewBox="0 0 16 16" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><path d="M3 14V9M7.5 14V4M12 14V7"/></svg>Weekly</a>
    <a href="hub.html" class="on"><svg class="ic" viewBox="0 0 16 16" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12l4-4 3 3 5-6"/><path d="M10.5 4H14v3.5"/></svg>Monthly + AI</a>
    <a href="guide.html"><svg class="ic" viewBox="0 0 16 16" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3.4c1.2-.7 2.7-1 4-.8 1 .1 1.6.5 2 .8v9.2c-.4-.3-1-.7-2-.8-1.3-.2-2.8.1-4 .8V3.4z"/><path d="M14 3.4c-1.2-.7-2.7-1-4-.8-1 .1-1.6.5-2 .8v9.2c.4-.3 1-.7 2-.8 1.3-.2 2.8.1 4 .8V3.4z"/></svg>Guide</a>
  </div>
</div>
<div class="wrap">
