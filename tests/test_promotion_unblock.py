...
class TestTransportIsNotAVerdict:
    ...
    url_validator saw SSLError -> Status=rejected; claude_check saw the SAME
    SSLError and counted it as a second reviewer confirming a dead URL ->
    archived. One infrastructure fault, counted twice.
    """

    TRANSIENT = [
        "fetch failed (fetch error: SSLError: HTTPSConnectionPool("
        "host='rappel.conso.gouv.fr', port=443): Max retries exceeded",
    ...
    @staticmethod
    def _escalates(reason):
        # Calls the REAL production predicate, not a copy, so reverting the
        # fix makes these tests fail.
    ...
    def test_transport_failure_never_archives_a_row(self, reason):
        assert not self._escalates(reason), (
            f"transport failure treated as a reviewer verdict: {reason!r}")
...
class TestUrlValidatorTolerance:
    def test_tls_error_is_tolerated_not_broken(self, monkeypatch):
        ...
        monkeypatch.setattr(uv.requests, "get", _boom, raising=False)
        monkeypatch.setattr(uv.requests, "head", _boom, raising=False)
        res = uv.check_url("https://rappel.conso.gouv.fr/fiche-rappel/22999/Interne")
