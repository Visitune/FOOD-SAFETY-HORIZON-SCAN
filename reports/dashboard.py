...
Reads ALL records from SQLite DB (fsis_global.db) and generates docs/index.html.

STATUS (audit 2026-07-30): this script and fsis_global.db are NOT part of
the production pipeline. Verified by repo-wide search: no
.github/workflows/*.yml invokes dashboard.py or references fsis_global.db,
and no other .py file reads/writes fsis_global.db. The actual production
dashboard (docs/index.html as published on GitHub Pages) is generated from
docs/data/recalls.xlsx via the scrape -> gate -> merge -> report chain
documented in pipeline/. Keep this file only if you intend to wire it back
...
from reports._theme import esc, tier_number, SRC_CSS
...
def _stats(rows):
    ...
    tier1 = sum(1 for r in rows if tier_number(r.get("pathogen_ai") or r.get("pathogen",""))==1)
    return {
        "total": len(rows),
        "tier1": tier1,
        "outbreaks": outbreaks,
...
def generate(db_path="fsis_global.db"):
    ...
    rows.sort(key=lambda r: (
        tier_number(r.get("pathogen_ai") or r.get("pathogen","")),
