...
"""
import os, json, logging, statistics, sys
...
from reports._theme import esc, pathogen_color, FONTS_LINK, dark_footer
...
def _build_html(month_name, month_key, year, st, recalls, preds, trend,
    ...
    def path_rows():
        return "".join(
            ...
            f'<span style="font-size:13px;color:{pathogen_color(p["p"])};">{esc(p["p"] or "Unknown")}</span>'
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
  <span style="font-size:13px;color:{p['color']};font-weight:500;">{esc(p['pathogen'])}{peak_flag}</span>
...
<div style="font-size:10px;color:#555;margin-top:2px;font-family:monospace;">Historical avg {preds['next_month']}: {p['hist_avg']}/month &nbsp;·&nbsp; Peak: {p['peak_month_name']}</div>
    ...
    def recall_rows():
        ...
        for r in sorted(recalls[:15], key=lambda x: x.get("recall_date",""), reverse=True):
            ...
            col = pathogen_color(pathogen)
            html += f"""<tr>
<td style="font-family:monospace;font-size:11px;color:#666;">{esc(r.get('recall_date',''))}</td>
<td style="font-size:11px;">{esc(r.get('source',''))}</td>
<td style="font-weight:500;font-size:12px;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{esc(r.get('firm','')[:50])}</td>
<td style="font-size:11px;color:#bbb;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{esc(r.get('product','')[:55])}</td>
<td style="color:{col};font-weight:700;font-size:11px;font-family:monospace;">{esc(pathogen)}</td>
<td style="font-size:11px;color:#666;">{esc(r.get('country',''))}</td>
</tr>"""
        return html

    narrative_html = "".join(
        f'<p style="margin-bottom:14px;line-height:1.75;font-size:15px;color:#ddd;">{esc(p)}</p>'
        for p in narrative.split("\n\n") if p.strip()
    )

    elevated_tags = "".join(
        f'<span style="background:rgba(232,96,26,.12);border:1px solid rgba(232,96,26,.3);color:{p["color"]};padding:4px 12px;border-radius:3px;font-size:12px;font-family:monospace;font-weight:700;margin:3px;">{esc(p["pathogen"])} {p["trend"]} {p["risk_score"]}/10</span>'
