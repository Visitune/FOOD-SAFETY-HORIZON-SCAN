...
"""
import json, logging, statistics, sys
...
def generate():
    ...
    def path_5yr_rows():
        ...
        return "".join(
            ...
            f'<span style="color:{pathogen_color(p["p"])};font-size:13px;">{esc(p["p"] or "Unknown")}</span>'
            ...
            f'<div style="background:{pathogen_color(p["p"])};width:{round(p["c"]/total*100)}%;height:100%;border-radius:2px;"></div></div></div>'
    ...
    def cat_rows():
        return "".join(
            ...
            f'<span style="font-size:12px;">{esc(c["h"])}</span>'
    ...
    def top_recalls_rows():
        ...
        return "".join(
            f'<tr><td style="font-family:monospace;font-size:11px;color:#666;">{esc(r.get("recall_date",""))}</td>'
            f'<td style="font-size:11px;">{esc(r.get("source",""))}</td>'
            f'<td style="font-weight:500;font-size:12px;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{esc(r.get("firm","")[:50])}</td>'
            f'<td style="font-size:11px;color:#bbb;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{esc(r.get("product","")[:55])}</td>'
            f'<td style="color:{pathogen_color(r.get("pathogen_ai") or r.get("pathogen",""))};font-weight:700;font-size:11px;font-family:monospace;">{esc(r.get("pathogen_ai") or r.get("pathogen","—"))}</td>'
            f'<td style="font-size:11px;color:#666;">{esc(r.get("country",""))}</td></tr>'
            for r in top
        )
...
<title>Food Safety Horizon Scanning Yearly Report {year}</title>
{FONTS_LINK}
<style>
...
<body><h1>Food Safety Horizon Scanning · Yearly Reports</h1><ul>{link}</ul></body></html>""")
