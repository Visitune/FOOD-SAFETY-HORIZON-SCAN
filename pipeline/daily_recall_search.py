...
# ---------------------------------------------------------------------------
SYSTEM_PROMPT = (
    "You are a senior food-safety operations analyst for VisiPilot, a Greek "
...
<body><div class="wrap">
<a class="back" href="https://food-safety-horizon-scan.vercel.app/">← Dashboard</a>
<div class="brand">VisiPilot · Food Safety Intelligence · Daily Brief</div>
...
Allergen-only, labeling, quality issues excluded per VisiPilot scope.<br>
<a href="https://food-safety-horizon-scan.vercel.app/">Back to dashboard</a> · <a href="../daily-index.json">JSON archive</a>
</div>
</div></body></html>
"""
...
def render_daily_html(target_date: date, recalls: List[Recall],
    ...
    if not recalls:
        body = ('<div class="empty">No pathogen / biotoxin / foreign-material / '
                ...
                '<br>(Allergen-only recalls are excluded per VisiPilot scope.)</div>')
    else:
        # Group by region
